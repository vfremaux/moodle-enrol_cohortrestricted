...
require_once($CFG->dirroot.'/customscripts/enrol/manual/locallib.php');
...
switch ($action) {
    ...
    case 'searchcohorts':
        ...
        $addedenrollment = optional_param('enrolcount', 0, PARAM_INT);
        $perpage = optional_param('perpage', 25, PARAM_INT);  //  This value is hard-coded to 25 in quickenrolment.js
        $outcome->response = enrol_manual_get_potential_cohorts_filtered($context, $enrolid, $search, $page, $perpage, $addedenrollment);
        $outcome->success = true;
        break;
    case 'enrol':
        $enrolid = required_param('enrolid', PARAM_INT);
