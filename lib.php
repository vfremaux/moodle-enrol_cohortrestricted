 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_cohortrestricted_plugin extends enrol_plugin {
    ...
    public function can_delete_instance($instance) {
        ...
        return has_capability('enrol/cohortrestricted:config', $context);
    }
    ...
    public function add_instance($course, array $fields = null) {
        ...
        if (!empty($fields['customint2']) && $fields['customint2'] == COHORTRESTRICTED_CREATE_GROUP) {
            // Create a new group for the cohort if requested.
            $context = context_course::instance($course->id);
            require_capability('moodle/course:managegroups', $context);
            $groupid = enrol_cohortrestricted_create_new_group($course->id, $fields['customint1']);
    ...
    public function update_instance($instance, $data) {
        ...
        if ($data->roleid != $instance->roleid) {
            ...
            $params = array(
                ...
                'component' => 'enrol_cohortrestricted',
                'itemid' => $instance->id
            );
            role_unassign_all($params);
        }
        // Create a new group for the cohort if requested.
        if ($data->customint2 == COHORTRESTRICTED_CREATE_GROUP) {
            require_capability('moodle/course:managegroups', $context);
            $groupid = enrol_cohortrestricted_create_new_group($instance->courseid, $data->customint1);
    ...
    protected function process_data($input) {
        ...
        if ($matches = preg_match_all('/%PROFILE_FIELD_(.*?)%/', $input, $matches)) {
            foreach ($matches as $matchset) {
                $profilekey = $matchset[0];
                $fieldname = $matchset[1];
                if ($fieldid = $DB->get_field('user_info_field', 'id', array('shortname' => $fieldname))) {
                    $params = array('userid' => $USER->id, 'fieldid' => $fieldid);
                    $value = $DB->get_field('user_info_data', 'value', $params);
                } else {
                    // If not found in profile fields, try standard \'user\' fields.
                ...
                $input = str_replace($profilekey, $value, $input);
            }
        }
    ...
    protected function get_group_options($coursecontext) {
        ...
        if (has_capability('moodle/course:managegroups', $coursecontext)) {
            $groups[COHORTRESTRICTED_CREATE_GROUP] = get_string('creategroup', 'enrol_cohortrestricted');
        }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $coursecontext) {
        ...
        */
        ...
        $mform->addElement('select', 'customint2', get_string('addgroup', 'enrol_cohortrestricted'), $groups);
    }
 ...
 * @return bool
 */
function enrol_cohortrestricted_allow_group_member_remove($itemid, $groupid, $userid) {
    return false;
}
 ...
 */
function enrol_cohortrestricted_create_new_group($courseid, $cohortid) {
    global $DB, $CFG;
    ...
    $a->name = $groupname;
    $a->increment = '';
    $groupname = trim(get_string('defaultgroupnametext', 'enrol_cohortrestricted', $a));
    $inc = 1;
